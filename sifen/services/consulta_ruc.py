...
class ConsultaRUCService(SifenServiceBase):
    ...
    def consultar_ruc(self, ruc: str, dv: str) -> RespuestaConsultaRUC:
        ...
        r_cons_ruc = etree.SubElement(
            soap_body,
            f"{{{NAMESPACE_SIFEN}}}rConsRuc"
        )
        
        # dId - Identificador de la consulta
        d_id = etree.SubElement(r_cons_ruc, f"{{{NAMESPACE_SIFEN}}}dId")
        d_id.text = "1"
        
        # dRUCCons - RUC a consultar
        d_ruc_cons = etree.SubElement(r_cons_ruc, f"{{{NAMESPACE_SIFEN}}}dRUCCons")
        d_ruc_cons.text = f"{ruc}-{dv}"
    ...
    def _process_response(
        ...
        resp_elem = body.find('.//*[local-name()="rRetConsRuc"]')
        
        if resp_elem is None:
            raise SifenException(
                "No se encontró elemento rRetConsRuc en la respuesta"
            )
        ...
        x_cont_ruc = resp_elem.find('.//*[local-name()="xContRUC"]')
        
        if x_cont_ruc is not None:
            ruc_text = x_cont_ruc.findtext('.//*[local-name()="dRUCCons"]', '')
            ...
            contribuyente = DatosContribuyente(
                ruc=ruc,
                dv=dv,
                nombre=x_cont_ruc.findtext('.//*[local-name()="dNombCons"]', ''),
                tipo_contribuyente=x_cont_ruc.findtext('.//*[local-name()="dTipCont"]'),
                estado=x_cont_ruc.findtext('.//*[local-name()="dEstCont"]'),
            )
